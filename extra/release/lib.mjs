...
export function getRepoNames() {
    ...
    return ["taha-adel/uptime-kuma", "ghcr.io/taha-adel/uptime-kuma"];
}
 ...
 * docker buildx build -f docker/dockerfile --platform linux/amd64 -t taha-adel/uptime-kuma:upload-artifact --build-arg VERSION --build-arg GITHUB_TOKEN --target upload-artifact . --progress plain
 * @param {string} version Version
...
export function uploadArtifacts(version, githubToken) {
    let args = [
        ...
        "linux/amd64",
        "-t",
        "taha-adel/uptime-kuma:upload-artifact",
        "--build-arg",
        `VERSION=${version}`,
        "--build-arg",
        "GITHUB_TOKEN",
